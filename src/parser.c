...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[oriented_region]")==0) return ORIENTED_REGION;
...
layer parse_oriented_region(list *options, size_params params)
{
    int coords = option_find_int(options, "coords", 4);
    int classes = option_find_int(options, "classes", 20);
    int num = option_find_int(options, "num", 1);

    layer l = make_oriented_region_layer(params.batch, params.w, params.h, num, classes, coords);
    printf("%i %i\n", l.outputs, params.inputs);
    assert(l.outputs == params.inputs);
...
network parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        }else if(lt == ORIENTED_REGION){
            l = parse_oriented_region(options, params);
